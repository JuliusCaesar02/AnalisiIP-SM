	...
	private void initialize() {
		...
		public void actionPerformed(ActionEvent e) {
			...
			if(sub.decon(sub.setIp(ip))[0] == -1) {
				textArea.setText("È stata riscontrata un'eccezione! Verifica il formato dell'IP" +"\n");
				return;	
			}		
			...
			if(sub.decon(sub.setIp(sm))[0] == -1) {
				textArea.setText("È stata riscontrata un'eccezione! Verifica il formato della SM" +"\n");
				return;	
			}
	    	...
	    	if(sub.setClasse() == 'D' || sub.setClasse() == 'E') {
	    		 textArea.append("Non è possibile eseguire ulteriori operazioni" +"\n");	 		
	    		 return;	
		    }
			if(sub.setClasse() == 'X') {
